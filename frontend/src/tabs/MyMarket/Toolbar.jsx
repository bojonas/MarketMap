...
export default function Toolbar({ setEditMode }) {
  ...
  const handleSave = async () => {
    setEditMode(false);
    if (checkChanges(mapLayout.map_layout, market.map_layout)) return alert('No Changes');
    mapLayout.build(mapLayout.map_layout, mapLayout.zones);
    alert(await requestUpdateMapLayout(user_id, mapLayout.map_layout));
  }
  ...
  return (
    <div className='flex flex-col items-center text-center w-full h-full bg-purple-custom gap-[5%]'>
      ...
      <div onClick={() => null} className='flex items-center justify-center rounded-full p-[3%] bg-offwhite border-offwhite border-[0.3svh] hover:border-darkgray-custom text-black h-[5.5svh] text-[2.2svh] cursor-pointer'>
          <SlFrame size={24}/>
          <p className='ml-[0.5svw]'>Edit Zones</p>
      </div>
      <div className='w-full h-full flex gap-[5%] items-center justify-center content-center float-end end-full'>
        <div onClick={() => setEditMode(false)} className='custom-button gap-[10%] bg-darkgray-custom border-darkgray-custom hover:border-offwhite h-[5.5svh] text-[2.2svh] cursor-pointer'>
          <IoArrowBack size={25}/>
          <p>Back</p>
        </div>
